...
> Superseded governance note: this is a historical strategy optimization plan.
> Runtime boundaries and active hard gates are now defined by
...
- `pytest openclaw_tests/unified/test_unified_strategy_recommendation_service.py`
...
- `tools/archive/research/strategy_optimization_stage_audit.py`
...
- CLI：`tools/archive/strategy_competition/strategy_competition_portfolio_audit.py`
...
可使用 `tools/archive/strategy_competition/record_strategy_competition_shadow_feedback.py` 录入 shadow order 的真实回报并生成
...
可使用 `tools/archive/strategy_competition/check_strategy_competition_live_order_authority.py` 在任何 broker/live submission 前执行实盘订单出口
...
可使用 `tools/archive/strategy_competition/check_strategy_competition_broker_submission_guard.py` 在 broker adapter 前执行提交守卫。该层要求
