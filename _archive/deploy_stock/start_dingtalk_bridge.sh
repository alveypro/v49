...
HOST="${DINGTALK_BRIDGE_HOST:-0.0.0.0}"
PORT="${DINGTALK_BRIDGE_PORT:-8601}"

# Force cloud brain only. Never fallback to local weak answers.
export OPENCLAW_CLOUD_BRAIN_ONLY=1
...
_validate_cloud_endpoint() {
  ...
  if [[ ! "$u" =~ ^https?:// ]]; then
    echo "[dingtalk-bridge] ERROR: ${label} must start with http:// or https:// -> $u"
    exit 2
  fi
  if [[ "$u" != *"/api/ai/chat"* ]]; then
    echo "[dingtalk-bridge] ERROR: ${label} must contain /api/ai/chat -> $u"
    exit 2
  fi
}
...
# Require and validate cloud API endpoint configuration before startup.
_has_endpoint=0
...
if [[ "${_has_endpoint}" -eq 0 ]]; then
  echo "[dingtalk-bridge] ERROR: missing cloud brain endpoint."
...
echo "[dingtalk-bridge] starting at ${HOST}:${PORT}"
exec "$PYTHON_BIN" deploy_stock/dingtalk_bridge_server.py
