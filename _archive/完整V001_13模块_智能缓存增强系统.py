...
try:
    ...
    logger.error("❌ 机构数据模块未找到，系统拒绝使用演示数据")
...
class V001CompleteSystem:
    ...
    def get_real_stock_data(self, count=20):
        ...
        if UNIFIED_DATA_MODULE_AVAILABLE:
            try:
                ...
                st.error(f"❌ 机构数据获取失败: {e}")
                st.error("🚫 系统拒绝使用演示数据，请检查机构数据模块")
                return pd.DataFrame()  # 返回空DataFrame而不是演示数据
        else:
            st.error("❌ 机构数据模块未安装")
            st.error("🚫 系统拒绝使用演示数据，请安装24小时增强版机构数据模块")
            return pd.DataFrame()
    ...
    def render_home(self):
        ...
        with col3:
            data_status = "✅ 机构数据" if UNIFIED_DATA_MODULE_AVAILABLE else "❌ 未连接"
    ...
    def render_strategy_backtest(self):
        ...
        if st.button("开始回测"):
            ...
            backtest_data = pd.DataFrame({
                ...
                '基准收益率': (1 + [0.01] * len(dates)).cumprod()
            })
    ...
    def render_system_settings(self):
        ...
        # 数据源设置
        st.subheader("📊 数据源设置")
        data_source = st.selectbox("主数据源", ["机构数据模块", "Tushare Pro", "演示数据"]) # 已删除演示数据选项
