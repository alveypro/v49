...
def _reconciliation_failures(recon: JsonDict, *, feedback: JsonDict) -> List[str]:
    ...
    if _clean_text(recon.get("execution_feedback_review_hash")) != _clean_text(feedback.get("broker_execution_feedback_review_hash")):
...
def build_strategy_competition_post_rerun_post_trade_reconciliation(
    conn: sqlite3.Connection,
    *,
    post_rerun_broker_execution_feedback_review_artifact_path: str | Path,
    post_trade_reconciliation_input_artifact_path: str | Path = "",
    output_dir: str | Path,
    operator_name: str = "strategy_competition_post_rerun_post_trade_reconciliation",
) -> JsonDict:
    """Reconcile post-rerun execution feedback without granting lifecycle completion."""

    apply_professional_migrations(conn)
    feedback = _load_json(post_rerun_broker_execution_feedback_review_artifact_path)
    recon = _load_json(post_trade_reconciliation_input_artifact_path)
    feedback_hash = _hash_file(post_rerun_broker_execution_feedback_review_artifact_path)
    recon_hash = _hash_file(post_trade_reconciliation_input_artifact_path)
    blocking: List[str] = []
    if feedback.get("broker_execution_feedback_review_status") != "post_rerun_broker_execution_feedback_ready_for_post_trade" or feedback.get("passed") is not True:
        blocking.append("post_rerun_broker_execution_feedback_review_not_ready")
    ...
    payload: JsonDict = {
        ...
        "reconciliation_status": "post_rerun_post_trade_reconciliation_passed" if passed else "post_rerun_post_trade_reconciliation_blocked",
        "passed": passed,
        "trade_lifecycle_complete": passed,
        "source_post_rerun_broker_execution_feedback_review_artifact": str(post_rerun_broker_execution_feedback_review_artifact_path),
        "source_post_rerun_broker_execution_feedback_review_hash": feedback_hash,
        "post_trade_reconciliation_input_artifact": str(post_trade_reconciliation_input_artifact_path or ""),
        "post_trade_reconciliation_input_hash": recon_hash,
        "source_artifact_hashes": {
            "broker_execution_feedback_review": feedback_hash,
            "post_trade_reconciliation_input": recon_hash,
        },
        ...
        canonical_json(
            {
                ...
                "source_artifact_hashes": {
                    "broker_execution_feedback_review": payload["source_post_rerun_broker_execution_feedback_review_hash"],
                    "post_trade_reconciliation_input": payload["post_trade_reconciliation_input_hash"],
                },
    ...
    path = output / f"strategy_competition_post_rerun_post_trade_reconciliation_{payload['competition_run_id'] or 'unknown'}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
