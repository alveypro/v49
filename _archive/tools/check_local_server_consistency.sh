...
for f in "${FILES[@]}"; do
  ...
  remote_sha="$(sshpass -p "$DEPLOY_PASS" ssh -o StrictHostKeyChecking=no "$DEPLOY_HOST" "sha256sum \"$REMOTE_APP_DIR/$f\" 2>/dev/null | cut -d' ' -f1" || true)"
...
dups="$(sshpass -p "$DEPLOY_PASS" ssh -o StrictHostKeyChecking=no "$DEPLOY_HOST" "
