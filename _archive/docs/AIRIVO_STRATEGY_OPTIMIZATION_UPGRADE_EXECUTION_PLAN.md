...
- `_archive/openclaw/config/strategy_center.yaml`
...
- `_archive/openclaw/services/strategy_backtest_diagnostic_service.py`
- `_archive/openclaw/research/backtest_param_sweep.py`
...
- `_archive/openclaw/runtime/v49_handlers.py`
- `_archive/openclaw/runtime/v8_signal_evaluator.py`
- `_archive/openclaw/runtime/combo_signal_evaluator.py`
...
- `_archive/openclaw/runtime/v6_backtest_context.py`
- `_archive/openclaw/runtime/v49_handlers.py`
- `_archive/openclaw/services/experiment_governance_service.py`
...
- `_archive/openclaw/services/unified_strategy_recommendation_service.py`
...
- `_archive/openclaw/services/execution_evidence_service.py`
- `_archive/openclaw/services/broker_execution_report_service.py`
...
- `_archive/openclaw/services/strategy_optimization_stage_service.py`
- `_archive/openclaw/services/rejected_backtest_artifact_ledger_service.py`
- `_archive/tools/strategy_optimization_stage_audit.py`
- `_archive/tools/rejected_backtest_artifacts.py`
- `_archive/tools/governance_gate.py`
...
python _archive/tools/governance_gate.py
```

记录 rejected artifact：

```bash
python _archive/tools/rejected_backtest_artifacts.py record \
...
```bash
python _archive/tools/backfill_execution_attribution.py \
...
- 服务：`_archive/openclaw/services/strategy_competition_audit_service.py`
- CLI：`_archive/tools/strategy_competition_portfolio_audit.py`
- 当前事实审计生成器：`_archive/tools/build_current_strategy_competition_audit.py`
...
可使用 `_archive/tools/build_strategy_competition_shadow_execution_plan.py` 从已通过 pre-trade 的 competition audit 生成
...
可使用 `_archive/tools/record_strategy_competition_shadow_feedback.py` 录入 shadow order 的真实回报并生成
...
可使用 `_archive/tools/build_strategy_competition_independent_validation.py` 对外部独立验证人报告做系统级复核。该工具不允许
...
可使用 `_archive/tools/build_strategy_competition_production_readiness.py` 生成最终发布前 readiness artifact。该闸只接受
...
可使用 `_archive/tools/build_strategy_competition_operational_controls.py` 先生成标准化运行控制包。该工具会校验 kill switch、
...
可使用 `_archive/tools/build_strategy_competition_evidence_intake_packet.py` 生成证据补齐包。该包汇总当前 audit、shadow、
...
可使用 `_archive/tools/review_strategy_competition_evidence_submission.py` 对填报后的补证文件做 intake submission review。
...
可使用 `_archive/tools/build_strategy_competition_formal_validation_handoff.py` 在 submission review 通过后生成
...
可使用 `_archive/tools/review_strategy_competition_formal_validation_results.py` 回收并复核 handoff 后的正式验证输出。该 review
...
可使用 `_archive/tools/build_strategy_competition_human_release_approval.py` 生成最终人工发布批准 artifact。该层只在 formal
...
可使用 `_archive/tools/check_strategy_competition_live_order_authority.py` 在任何 broker/live submission 前执行实盘订单出口
...
可使用 `_archive/tools/check_strategy_competition_broker_submission_guard.py` 在 broker adapter 前执行提交守卫。该层要求
...
可使用 `_archive/tools/review_strategy_competition_broker_submission_response.py` 复核 broker adapter 的提交回报。该层要求
...
可使用 `_archive/tools/review_strategy_competition_broker_execution_feedback.py` 复核最终 broker execution feedback。该层要求
...
可使用 `_archive/tools/reconcile_strategy_competition_post_trade.py` 做 post-trade reconciliation。该层要求 broker execution
...
可使用 `_archive/tools/adjudicate_strategy_competition_trade_lifecycle.py` 生成 post-release trade lifecycle 的 court-of-record。
...
可使用 `_archive/tools/build_strategy_competition_evidence_chain_manifest.py` 生成全链路 evidence-chain manifest。该层把
...
可使用 `_archive/tools/build_strategy_competition_evidence_remediation_work_order.py` 从 evidence-chain manifest 生成补证工单。
...
可使用 `_archive/tools/build_strategy_competition_remediation_closure_submission.py` 从补证工单和已完成的 validator artifact 生成
...
可使用 `_archive/tools/review_strategy_competition_remediation_closure.py` 审核补证工单关闭提交。该层校验 closure submission
...
可使用 `_archive/tools/build_strategy_competition_formal_rerun_plan.py` 在 closure review accepted 后生成固定顺序的 formal
...
可使用 `_archive/tools/build_strategy_competition_formal_rerun_output_submission.py` 从 formal rerun plan 和逐步输出 artifact
...
可使用 `_archive/tools/review_strategy_competition_formal_rerun_results.py` 回收并审核 formal rerun 输出。该层要求 rerun plan
...
可使用 `_archive/tools/build_strategy_competition_rerun_court_rebuild_submission.py` 从 accepted formal rerun result review、重建的
...
可使用 `_archive/tools/review_strategy_competition_rerun_court_rebuild.py` 审核 formal rerun 后的 court-of-record 重建结果。
...
可使用 `_archive/tools/build_strategy_competition_post_rerun_release_readiness_submission.py` 先把 rerun 后发布就绪证据打包成
...
可使用 `_archive/tools/review_strategy_competition_post_rerun_release_readiness.py` 做 rerun 后发布就绪复核。该层要求
...
可使用 `_archive/tools/build_strategy_competition_post_rerun_live_authority_submission.py` 先把 rerun 后 live authority
...
可使用 `_archive/tools/review_strategy_competition_post_rerun_live_authority.py` 复核 rerun 后 live authority 出口。该层要求
...
可使用 `_archive/tools/build_strategy_competition_post_rerun_broker_guard_submission.py` 先把 rerun 后 broker guard
...
可使用 `_archive/tools/review_strategy_competition_post_rerun_broker_guard.py` 复核 rerun 后 broker guard 出口。该层要求
...
可使用 `_archive/tools/review_strategy_competition_post_rerun_broker_response.py` 复核 rerun 后 broker response。该层要求
...
可使用 `_archive/tools/review_strategy_competition_post_rerun_broker_execution_feedback.py` 复核 rerun 后 broker execution
...
可使用 `_archive/tools/reconcile_strategy_competition_post_rerun_post_trade.py` 复核 rerun 后 post-trade reconciliation。该层要求
...
可使用 `_archive/tools/adjudicate_strategy_competition_post_rerun_trade_lifecycle.py` 复核 rerun 后 trade lifecycle。
...
可使用 `_archive/tools/build_strategy_competition_post_rerun_evidence_chain_manifest.py` 生成 post-rerun evidence-chain manifest。
...
`_archive/tools/review_strategy_competition_post_rerun_human_release_approval_review.py` 作为指定 validator。

可使用 `_archive/tools/review_strategy_competition_post_rerun_human_release_approval_review.py` 复核 post-rerun human release
...
可使用 `_archive/tools/adjudicate_strategy_competition_release_chain.py` 生成
