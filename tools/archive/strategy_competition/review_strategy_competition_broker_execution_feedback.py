...
from openclaw.services.strategy_competition_broker_execution_feedback_service import (  # noqa: E402
    build_strategy_competition_broker_execution_feedback_review,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Review broker execution feedback after strategy competition broker submission.")
    ...
    parser.add_argument("--output-dir", default="logs/openclaw/strategy_competition_broker_execution_feedback")
    parser.add_argument("--operator-name", default="strategy_competition_broker_execution_feedback_review")
    args = parser.parse_args()

    conn = sqlite3.connect(str(args.db_path or default_db_path()), timeout=30)
    try:
        payload = build_strategy_competition_broker_execution_feedback_review(
            conn,
