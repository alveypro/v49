...
from openclaw.services.strategy_competition_post_rerun_live_authority_submission_service import (  # noqa: E402
    build_strategy_competition_post_rerun_live_authority_submission,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Package post-rerun live authority evidence before review.")
    ...
    parser.add_argument("--output-dir", default="logs/openclaw/strategy_competition_post_rerun_live_authority_submission")
    parser.add_argument("--operator-name", default="strategy_competition_post_rerun_live_authority_submission")
    args = parser.parse_args()

    conn = sqlite3.connect(str(args.db_path or default_db_path()), timeout=30)
    try:
        payload = build_strategy_competition_post_rerun_live_authority_submission(
            conn,
