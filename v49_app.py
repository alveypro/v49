...
def _airivo_session_meta() -> Dict[str, str]:
    auth_inject_headers()
    auth_user = auth_get_user_info()
    return {
        "username": auth_user.get("username", ""),
        "display_name": auth_user.get("display_name", ""),
        "role": auth_user.get("role", "viewer"),
    }


def _airivo_has_role(required_role: str) -> bool:
    current_role = auth_get_user_role()
    return AIRIVO_ROLE_RANK.get(current_role, 0) >= AIRIVO_ROLE_RANK.get(str(required_role or "viewer").strip().lower(), 0)
